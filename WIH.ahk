...
Version=WIH v1.0.0			;current verison for update
;************************************************STYLE COLOR**********************************************
...
Hotkey, RWin, disable
	...
	IniRead, UpdateAtStart, %A_WorkingDir%\wih.ini, Others, UpdateAtStart, 0
	
	IniRead, RunAtStart, %A_WorkingDir%\wih.ini, Others, RunAtStart, 0
	if RunAtStart
	FileCreateShortcut , %A_ScriptDir%\%A_ScriptName%, %A_Startup%\WIH.lnk
	else{
	IfExist %A_Startup%\WIH.lnk 
	FileDelete, %A_Startup%\WIH.lnk
	}
...
UpdateCheck:
UrlDownloadToFile, https://wih.auct.eu/latestwih.html, %A_Temp%\latestwih.html
FileReadLine, NetVer, %A_Temp%\latestwih.html, 1
If (Version <> NetVer)
{
...
IfMsgBox Yes
	run, https://wih.auct.eu/download.html
}
else
   MsgBox 64, Info ,Your WIH is up to date!,2
return
...
IniWrite, %RunatStart%, %A_WorkingDir%\wih.ini, Others, RunatStart
menu, options, togglecheck, Run at Start
return

SetUpdateatStart:
UpdateatStart:=!UpdateatStart
IniWrite, %UpdateatStart%, %A_WorkingDir%\wih.ini, Others, UpdateatStart
